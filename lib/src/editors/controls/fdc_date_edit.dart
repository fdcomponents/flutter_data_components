...
import '../../data/fields/fdc_date_field.dart';
import '../core/fdc_bound_editor.dart';
import '../core/fdc_editor_descriptor.dart';

/// Data-aware editor for date-only dataset fields.
///
/// The control integrates text entry, date formatting, validation, and the
/// dataset edit lifecycle.
class FdcDateEdit extends StatelessWidget {
  /// Creates a [FdcDateEdit].
  const FdcDateEdit({
    super.key,
  ...
  Widget build(BuildContext context) {
    return FdcBoundEditor<DateTime, FdcDateField>(
      dataSet: dataSet,
      fieldName: fieldName,
      resolveBinding: (dataSet, fieldName) =>
          FdcFieldBindingResolver.resolve<FdcDateField>(
            dataSet,
            fieldName,
            ownerName: 'FdcDateEdit',
          ),
      fieldBuilder: (binding) => FdcDateEditorDescriptor<DateTime>(
        fieldName: fieldName,
