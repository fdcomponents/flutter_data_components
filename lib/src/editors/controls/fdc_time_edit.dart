...
import '../../data/fields/fdc_time_field.dart';
...
///
/// The control integrates text entry, time formatting, validation, and the
/// dataset edit lifecycle.
class FdcTimeEdit extends StatelessWidget {
  ...
  final FdcFieldFocusCallback<FdcTime>? onEnter;

  /// Called when focus leaves this bound field.
  final FdcFieldFocusCallback<FdcTime>? onExit;

  /// Called before a proposed value change is committed.
  final FdcFieldValueChangingCallback<FdcTime>? onValueChanging;

  /// Called after an accepted value change has been applied.
  final FdcFieldValueChangedCallback<FdcTime>? onValueChanged;
  ...
  Widget build(BuildContext context) {
    return FdcBoundEditor<FdcTime, FdcTimeField>(
      dataSet: dataSet,
      fieldName: fieldName,
      resolveBinding: (dataSet, fieldName) =>
          FdcFieldBindingResolver.resolve<FdcTimeField>(
            dataSet,
            fieldName,
            ownerName: 'FdcTimeEdit',
          ),
      fieldBuilder: (binding) => FdcTimeEditorDescriptor<FdcTime>(
        fieldName: fieldName,
