...
Future<void> main() async {
  ...
  assert(persistentMap.containsKey('quantity'));
  assert(persistentMap.containsKey('price'));
  assert(!persistentMap.containsKey('total'));

  final fullMap = dataSet.toMaps(includeNonPersistent: true).single;
  assert(fullMap['total'] == '22.50'.decimal);

  final insert = dataSet.changeSet.inserts.single;
  assert(insert.values.containsKey('quantity'));
  assert(insert.values.containsKey('price'));
  assert(!insert.values.containsKey('total'));
  assert(!insert.changedFields.contains('total'));
  ...
  assert(persistentCalculated.toMaps().single['total'] == '10.00'.decimal);
}
